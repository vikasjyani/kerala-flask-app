...
- 🟡 **Payback period is never calculated** (Solar + BESS and every alternative) — `estimated_payback_years` is `0` for all 33 DB rows, no assignment exists in code, yet the report shows a "0 months" payback row and `info.html` advertises "payback calculations." Inputs (`capital_cost`/`upfront_cost`, computed `monthly_savings`) exist. **Not yet fixed** — the correct formula for a levelized Solar+BESS cost is a financial-modeling decision (see the chat for options); awaiting confirmation before writing misleading numbers into a government report.
