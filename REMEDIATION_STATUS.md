...
- 🟠 **Latin text vanished in Malayalam PDF reports** (names, districts, "Balanced", and units kWh/kg/CO₂/PM2.5/µg/m³/kW/m²) — the report font (Noto Sans Malayalam) has no Latin glyphs and reportlab has no per-glyph fallback, so those cells appeared blank (the user-reported "empty" Name/District/Priority). **Fixed** with `ml_markup`/`localize_matrix` wrapping Latin runs in the Latin font. Verified: residential + commercial ml reports now contain the Latin content and Malayalam, 0 missing-glyph warnings.
- 🔵 **Excess PDF whitespace** — `bottomMargin` was 90pt while the footer sits at y≤30pt, leaving ~60pt dead space per page. Reduced to 55pt; residential report 5 → 4 pages.
- 🟡 **Payback period is never calculated** (Solar + BESS and every alternative) — `estimated_payback_years` is `0` for all 33 DB rows, no assignment exists in code. Per decision, rather than compute a potentially-misleading figure (Solar+BESS's stored monthly cost is levelized, i.e. already amortizes the capital), the misleading **"0 months" payback row was removed** from the PDF report and the overstated `info.html` claims were corrected ("ROI & Payback calculation" → "Cost & savings comparison"; "NPV and payback calculations" → "NPV & levelized 25-year cost"). If a real payback metric is wanted later, the inputs (`capital_cost`/`upfront_cost`, computed `monthly_savings`, and the Solar+BESS grid-backup operating cost) are available.
- 🟡 **Malayalam vowel signs mis-placed in the PDF** (e.g. "പ്രൊഫൈൽ", "ഉപഭോഗം") — neither reportlab nor matplotlib shape complex scripts and no HarfBuzz/raqm is installed, so pre-base signs (െ േ ൈ) and two-part signs (ൊ ോ ൌ) drew on the wrong side of the cluster. **Fixed** with `reorder_malayalam()` (logical→visual order), applied once at the `tr()`/`localize_fuel_name` chokepoint so it covers report text *and* chart labels; non-Malayalam passes through untouched. Verified via a matplotlib raster proxy (same advance-based rendering model as the PDF). Residual: consonant *conjuncts* still show an explicit virama instead of the ligature form — that needs a real shaping engine (`uharfbuzz` + glyph-level drawing) and is out of scope for a no-new-dependency fix.
